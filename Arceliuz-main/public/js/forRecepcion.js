...
HOST_API = `https://domiburguer.com/api/`
...
async function renderisarResumenPedido() {
	...
	if (!adiciones) {
		...
		adiciones = adicionesRta.map(e => e.data)
	}
	let opcionsDiv = ``
		...
		for (const i in element) {
			...
			if (producto[0]?.type == 'Domicilio') {
				...
				const selectDiv = `
					...
					const divPrice = `
					<div>
						<input type="number" class="form-select-sm" style="width: 8rem;" id="InputPrecio${producto.type}Producto${i}" onchange="cambiarPrecio('InputPrecio${producto.type}Producto${i}')">
							
						<div id="divSelect${producto.type}${i}">${baddConADICIONES}</div>
					</div>
					`
				arrayDivSpan.push(crearDivSpan(selectDiv, producto.type, divPrice, producto))
	...
	function crearDivSpan(cantidad, nombre, precio, producto) {
		console.log('[crearDivSpan]', precio)
...
function cambiarPrecio(inputTex){
	console.log(inputTex);
	const inputText = document.getElementById(inputTex)
	console.log("🚀 ~ file: forRecepcion.js:459 ~ cambiarPrecio ~ inputText:", inputText.value)
